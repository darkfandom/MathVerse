...
/// <summary>
/// Provides evaluation and tessellation of B-spline surfaces defined by a control point grid, knot vectors, and degrees in each parametric direction.
/// </summary>
public sealed class BSplineSurfaceAdvanced
{
    ...
    private readonly int _spanCountU;
    private readonly int _spanCountV;

    /// <summary>
    /// Initializes a new instance of the <see cref="BSplineSurfaceAdvanced"/> class with control points, knot vectors, and degrees.
    /// </summary>
    /// <param name="controlPoints">An n×m grid of control points defining the surface.</param>
        ...
        int degreeV)
    {
        ...
        _degreeU = degreeU;
        _degreeV = degreeV;
        _spanCountU = n - degreeU;
        _spanCountV = m - degreeV;
    }

    /// <summary>
    /// Evaluates the surface position at parametric coordinates (u, v) using Cox-de Boor basis functions.
    /// </summary>
    ...
    public Point3D Evaluate(double u, double v)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            double bu = BasisFunction(_knotsU, i, _degreeU, clampedU);
            if (System.Math.Abs(bu) < 1e-15) continue;

            for (int j = 0; j < m; j++)
            {
                double bv = BasisFunction(_knotsV, j, _degreeV, clampedV);
                if (System.Math.Abs(bv) < 1e-15) continue;

                double w = bu * bv;
                x += w * _controlPoints[i][j].X;
                y += w * _controlPoints[i][j].Y;
                z += w * _controlPoints[i][j].Z;
            }
        }
    ...
    /// <summary>
    /// Evaluates the Cox-de Boor recursive basis function N_{i,p}(t) for the given knot vector, index, degree, and parameter value.
    /// </summary>
    ...
    /// <param name="t">The parameter value at which to evaluate.</param>
    /// <returns>The value of the basis function N_{i,degree}(t).</returns>
    internal static double BasisFunction(ImmutableArray<double> knots, int i, int degree, double t)
    {
        if (degree == 0)
        {
            bool leftClosed = (i + 1 < knots.Length && System.Math.Abs(knots[i] - knots[i + 1]) < 1e-15);
            bool rightClosed = (i > 0 && System.Math.Abs(knots[i] - knots[i - 1]) < 1e-15);
        ...
        if (System.Math.Abs(denom1) > 1e-15)
            term1 = (t - knots[i]) / denom1 * BasisFunction(knots, i, degree - 1, t);

        if (System.Math.Abs(denom2) > 1e-15)
            term2 = (knots[i + degree + 1] - t) / denom2 * BasisFunction(knots, i + 1, degree - 1, t);
