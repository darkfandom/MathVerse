...
/// <summary>
/// Implements ridge regression with L2 regularization to prevent overfitting.
/// </summary>
public sealed class RidgeRegression
{
    ...
    /// </summary>
    /// <param name="alpha">Regularization strength. Higher values increase regularization.</param>
    /// <exception cref="ArgumentException">Thrown when alpha is negative.</exception>
    ...
    /// <summary>
    /// Trains the ridge regression model using the modified normal equation: (X^T X + alpha * I)^-1 X^T y.
    /// </summary>
    ...
    public void Train(double[][] X, double[] y)
    {
        ...
        double[][] XtXReg = new double[p][];
        for (int i = 0; i < p; i++)
        {
            XtXReg[i] = new double[p];
            for (int j = 0; j < p; j++)
            {
                XtXReg[i][j] = XtX[i][j];
                if (i == j)
        ...
        double[][] XtXRegInverse = Inverse(XtXReg);
        _coefficients = MultiplyVector(XtXRegInverse, Xty);
    }
