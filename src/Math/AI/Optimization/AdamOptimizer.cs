...
/// <summary>Adam optimizer with bias-corrected first and second moment estimates.</summary>
public sealed class AdamOptimizer : IOptimizer
{
    /// <summary>Gets the name of this optimizer.</summary>
    public string Name => "Adam";

    /// <summary>Runs Adam optimization.</summary>
    ...
    public OptimizationResult Optimize(Func<double[], double> objective, double[] initial, OptimizationOptions? options = null)
    {
        ...
        for (iter = 1; iter <= opts.MaxIterations; iter++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                double mHat = m[i] / bc1;
                double vHat = v[i] / bc2;
                x[i] -= opts.LearningRate * mHat / (System.Math.Sqrt(vHat) + eps);
            }
